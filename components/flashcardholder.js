...
export default function FlashcardHolder({cards}) {
        ...
        <div className={styles.studyArea}>
            <div className={styles['card-holder']}>
                {cards.map((card, index) => (
                    ...
                    <Flashcard question={card.question} answer={card.answer} key={index} active={index===currentCard} side={side} onClick={() => flip_card()}></Flashcard>
                ))}
            </div>
            <div className={styles.flashcardButtonHolder}>
                ...
                <button className={styles.flashcardButton} onClick={nextCard}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="m12 4-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8-8-8z"/></svg>                
                </button>
            </div>
        </div>
